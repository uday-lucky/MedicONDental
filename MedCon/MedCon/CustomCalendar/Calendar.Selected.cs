...
namespace MedCon.CustomCalendar
{
	public partial class Calendar : ContentView
	{
		...
		protected void ChangeSelectedBorderWidth(int newValue, int oldValue)
		{
			...
			buttons.FindAll(b => b.IsSelected).ForEach(b => b.CornerRadius = newValue);
		}
		...
		{
			if (newValue == oldValue) return;
			buttons.FindAll(b => b.IsSelected).ForEach(b => b.OutlineColor = newValue);
		}
		...
		protected void SetButtonSelected(CalendarButton button, SpecialDate special)
		{
			Device.BeginInvokeOnMainThread(() =>
			{
				...
				button.CornerRadius = SelectedBorderWidth;
				button.OutlineColor = SelectedBorderColor;
