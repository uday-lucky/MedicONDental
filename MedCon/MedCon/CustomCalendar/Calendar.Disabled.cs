...
namespace MedCon.CustomCalendar
{
	public partial class Calendar : ContentView
	{
		...
		protected void ChangeDisabledBorderWidth(int newValue, int oldValue)
		{
			...
			buttons.FindAll(b => !b.IsEnabled).ForEach(b => b.CornerRadius = newValue);
		}
		...
		{
			if (newValue == oldValue) return;
			buttons.FindAll(b => !b.IsEnabled).ForEach(b => b.OutlineColor = newValue);
		}
		...
		protected void SetButtonDisabled(CalendarButton button)
		{
			Device.BeginInvokeOnMainThread(() =>
			{
				...
				button.CornerRadius = DisabledBorderWidth;
				button.OutlineColor = DisabledBorderColor;
