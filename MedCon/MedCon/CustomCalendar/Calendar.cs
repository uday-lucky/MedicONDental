...
namespace MedCon.CustomCalendar
{
	public partial class Calendar : ContentView
	{
        ...
        public Calendar()
		{
			...
			TitleLeftArrow = new Image
			{
                Source= "calendar_left_arrow.png",
                HeightRequest=25,
                WidthRequest=25
			};
			...
			TitleRightArrow = new Image
			{
                Source = "calendar_right_arrow",
                HeightRequest = 25,
                WidthRequest = 25
            };
            ...
            var tapGestureLeftArrow = new TapGestureRecognizer();
            tapGestureLeftArrow.Tapped += LeftArrowClickedEvent;
            TitleLeftArrow.GestureRecognizers.Add(tapGestureLeftArrow);

            var tapGestureRightArrow = new TapGestureRecognizer();
            tapGestureRightArrow.Tapped += RightArrowClickedEvent;
            TitleRightArrow.GestureRecognizers.Add(tapGestureRightArrow);
		...
		public static readonly BindableProperty BorderWidthProperty =
			BindableProperty.Create(nameof(BorderWidth), typeof(int), typeof(Calendar), Device.RuntimePlatform == Device.iOS ? 0 : 0,
		...
		protected void ChangeBorderColor(Color newValue, Color oldValue)
		{
			...
			buttons.FindAll(b => b.IsEnabled && !b.IsSelected).ForEach(b => b.OutlineColor = newValue);
		}
			...
			for (var i = 0; i < ShowNumOfMonths; i++)
			{
				...
				for (int r = 0; r < 6; r++)
				{
					for (int c = 0; c < 7; c++)
					{
                        ...
                        tapGestureRecognizer.Tapped += DateClickedEvent;
        ...
        protected void ChangeCalendar(CalandarChanges changes)
        {
			Device.BeginInvokeOnMainThread(() =>
			{
				...
				for (int i = 0; i < buttons.Count; i++)
				{
					...
					{
						buttons[i].LabelText = string.Format("{0}", start.Day);
					}
					else
					{
        ...
        protected void SetButtonNormal(CalendarButton button)
        {
            ...
            Device.BeginInvokeOnMainThread(() =>
            {
                ...
                button.CornerRadius = BorderWidth;
                button.OutlineColor = BorderColor;
